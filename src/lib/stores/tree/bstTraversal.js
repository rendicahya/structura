...
import { bstNodes, bstRootId } from './graphBST.js';
import { logOpBST } from '../shared/bstLog.js';
...
export function startTraversal(type) {
    ...
    const order = computeOrder(get(bstNodes), get(bstRootId), type);
    const method = METHODS[type];

    logOpBST(method.java, method.python, method.cpp);
...
export function startSearch(target) {
    ...
    const { path, foundId } = computeSearchPath(get(bstNodes), get(bstRootId), target);

    logOpBST(SEARCH_METHOD.java, SEARCH_METHOD.python, SEARCH_METHOD.cpp);
...
export function stepForward() {
    ...
    const node = get(bstNodes).find(n => n.id === nodeId);
    ...
    if (isLastStep) {
        if (state.type === 'search') {
            if (state.searchFound) {
                logOpBST('// FOUND', '# FOUND', '// FOUND');
                toast.success(`Found "${state.searchTarget}"`);
            } else {
                logOpBST('// NOT FOUND', '# NOT FOUND', '// NOT FOUND');
