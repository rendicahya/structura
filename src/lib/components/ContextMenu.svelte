<script>
  ...
  function handleRename() { editingVarName = true; setTimeout(() => varNameInput?.focus(), 10); }
  function handleEditData() { editingData = true; setTimeout(() => dataInput?.focus(), 10); }
  ...
  function handleDisconnectNext() { ondisconnectNext?.(); close(); }
  function handleDisconnectPrev() { ondisconnectPrev?.(); close(); }
  function handleSetHead() { onsetHead?.(); close(); }
  function handleSetTail() { onsetTail?.(); close(); }
  function handleSetWalk() { onsetWalk?.(); close(); }
  function handleUnlink() { onunlink?.(); close(); }
...
<style>
  .context-menu { position: fixed; z-index: 1000; background: var(--surface); border: 1px solid var(--border-bright); border-radius: 10px; padding: 6px; min-width: 190px; box-shadow: 0 8px 32px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.04); animation: menuIn 0.12s ease; }
  @keyframes menuIn { from { opacity: 0; transform: scale(0.95) translateY(-4px); } to { opacity: 1; transform: scale(1) translateY(0); } }
  .menu-header { padding: 4px 8px 8px; border-bottom: 1px solid var(--border); margin-bottom: 4px; display: flex; align-items: center; justify-content: space-between; gap: 6px; }
  .menu-node-label { font-family: var(--font-mono); font-size: 11px; color: var(--accent); font-weight: 500; }
  .menu-badges { display: flex; gap: 4px; }
  .badge { font-family: var(--font-mono); font-size: 9px; font-weight: 700; padding: 2px 5px; border-radius: 3px; letter-spacing: 0.5px; }
  ...
  .menu-item { display: flex; align-items: center; gap: 8px; width: 100%; padding: 7px 10px; background: none; border: none; border-radius: 6px; color: var(--text-dim); font-family: var(--font-ui); font-size: 13px; cursor: pointer; text-align: left; transition: all 0.1s; }
  ...
  .menu-item.head-item:hover { background: rgba(78,204,163,0.08); }
  .menu-item.tail-item { color: #c084fc; }
  .menu-item.tail-item:hover { background: rgba(192,132,252,0.08); }
  .menu-item.walk-item { color: #fb923c; }
  .menu-item.walk-item:hover { background: rgba(251,146,60,0.08); }
  .menu-divider { height: 1px; background: var(--border); margin: 4px 0; }
  .menu-input-row { padding: 6px 8px; display: flex; flex-direction: column; gap: 6px; }
  .menu-input-row label { font-size: 11px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px; }
  .menu-input-row input { background: var(--surface2); border: 1px solid var(--border-bright); border-radius: 6px; color: var(--text); font-family: var(--font-mono); font-size: 13px; padding: 6px 8px; outline: none; width: 100%; }
  .menu-input-row input:focus { border-color: var(--accent); box-shadow: 0 0 0 2px var(--accent-glow); }
  .btn-confirm { background: var(--accent); color: #fff; border: none; border-radius: 6px; padding: 5px 10px; font-family: var(--font-ui); font-size: 12px; font-weight: 600; cursor: pointer; align-self: flex-end; }
