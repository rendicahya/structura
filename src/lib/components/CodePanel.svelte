<script>
  ...
  // Tokenizer-based highlighter — supports both Java and Python
  const JAVA_KW   = new Set(['class','new','int','String','void','null','this','Node','return']);
  ...
  function highlight(raw, language) {
    ...
    while (i < raw.length) {
      ...
      if (/[a-zA-Z_]/.test(raw[i])) {
        ...
        tokens.push({ type: KW.has(word) ? 'keyword' : 'ident', text: word });
        i = j;
        continue;
      }
