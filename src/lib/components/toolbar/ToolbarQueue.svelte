<script>
    ...
    function confirmNewQueue() {
        ...
        clearLogQueue();
        initHistory();
        initQueue(newCapacity, newVarName.trim() || "queue");
        showNewQueue = false;
        toast.success(`Queue created with capacity ${newCapacity}`);
    }
    ...
    function handleDequeue() {
        if ($queueIsEmpty) {
            toast.error("Queue underflow — queue is empty");
            return;
        }
        pushHistory();
        dequeue();
        pushHistory();
        toast.success("Dequeued front element");
    }
    ...
    onMount(() => {
        ...
        window.addEventListener("queue:enqueue", onEnqueue);
        window.addEventListener("queue:dequeue", onDequeue);
        return () => {
            window.removeEventListener("queue:enqueue", onEnqueue);
...
{#if showNewQueue}
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="modal-overlay" onmousedown={() => (showNewQueue = false)}>
        <div class="modal" onmousedown={(e) => e.stopPropagation()}>
            ...
            <div class="modal-footer">
                <button
                    class="btn btn-secondary"
                    onclick={() => (showNewQueue = false)}>Cancel</button
                >
                <button class="btn btn-primary" onclick={confirmNewQueue}
                    >Create</button
                >
            </div>
        </div>
    ...
    <div class="modal-overlay" onmousedown={() => (showEnqueue = false)}>
        <div class="modal" onmousedown={(e) => e.stopPropagation()}>
            ...
            <div class="modal-footer">
                <button
                    class="btn btn-secondary"
                    onclick={() => (showEnqueue = false)}>Cancel</button
                >
                <button class="btn btn-primary" onclick={confirmEnqueue}
                    >Enqueue</button
                >
            </div>
        </div>
