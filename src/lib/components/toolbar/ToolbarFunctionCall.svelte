<script>
  ...
  registerHistoryHandlers(getSnapshotFC, applySnapshotFC);
  import { clearLogFC } from '../../stores/shared/functionCallLog.js';
  ...
  function handleNew() {
    if ($callStackState.steps.length > 0) {
      showConfirmNew = true;
    } else {
      confirmNewActual();
    }
  ...
  function confirmNewActual() {
    ...
    toast.success('Call stack reset');
  }

  function handleSave() {
    downloadStructure('function-call', getSnapshotFC());
...
<div class="toolbar">
  ...
  <div class="actions">
    ...
    <Tooltip text="Reset the call stack">
  ...
  <div class="modal-overlay" onmousedown={() => (showConfirmNew = false)}>
    <div class="modal modal-sm" onmousedown={(e) => e.stopPropagation()}>
      <div class="modal-header">
        <span class="modal-title">Reset call stack</span>
        <button class="close-btn" aria-label="Close" onclick={() => (showConfirmNew = false)}>
          <Icon name="close" size={14} />
        </button>
      </div>
      <div class="modal-body">
        <p class="confirm-text">Clear the current run and its call stack?</p>
      </div>
