<script>
  ...
  registerHistoryHandlers(getSnapshotRQ, applySnapshotRQ);
  import { clearLogRQ } from '../../stores/shared/requestQueueLog.js';
  ...
  // The server UI (CanvasRequestQueue) owns every interaction and dispatches
  // these events; here we only wrap them in the undo/redo bracket. The
  // auto-process loop dequeues straight from the store (not undoable), same
  // as the print-spooler demo.
  function handleSubmit({ method, path, ms }) {
    pushHistory();
    submitRequest(method, path, ms);
    pushHistory();
    toast.success(`Queued "${method} ${path}"`);
  }
  ...
  function handleProcess() {
    ...
    pushHistory();
    const req = handleFront();
    pushHistory();
    if (req) toast.success(`Handled "${req.method} ${req.path}"`);
  }
  ...
  function handleSave() {
    downloadStructure('request-queue', getSnapshotRQ());
  ...
  onMount(() => {
    ...
    const onProcess = () => handleProcess();
    window.addEventListener('requestqueue:submit', onSubmit);
    window.addEventListener('requestqueue:process', onProcess);
    return () => {
      window.removeEventListener('requestqueue:submit', onSubmit);
      window.removeEventListener('requestqueue:process', onProcess);
    };
  });
  ...
  <div class="modal-overlay" onmousedown={() => (showConfirmNew = false)}>
    <div class="modal modal-sm" onmousedown={(e) => e.stopPropagation()}>
      ...
      <div class="modal-body">
        <p class="confirm-text">Drop every request from the queue?</p>
      </div>
