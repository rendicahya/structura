<script>
  ...
  function confirmPush() {
    if (!pushValue.trim()) { toast.error('Value cannot be empty'); return; }
    pushHistory();
  ...
  function handlePop() {
    if ($linkedStackIsEmpty) { toast.error('Stack underflow — stack is empty'); return; }
    pushHistory();
    popLinkedStack();
    pushHistory();
  ...
  function handlePeek() {
    if ($linkedStackIsEmpty) { toast.error('Stack is empty'); return; }
    peekLinkedStack();
  }
  ...
  function handleSave() {
    ...
    a.href = url; a.download = 'structura-linked-stack-save.json'; a.click();
...
<div class="toolbar">
  ...
  <div class="actions">
    ...
    <Tooltip text={$linkedStackIsEmpty ? 'Stack is empty' : 'Pop top node from stack'}>
      <button class="btn btn-danger" onclick={handlePop} disabled={$linkedStackIsEmpty}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
    ...
    <Tooltip text={$linkedStackIsEmpty ? 'Stack is empty' : 'Peek top node'}>
      <button class="btn btn-secondary" onclick={handlePeek} disabled={$linkedStackIsEmpty}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
<style>
  ...
  .modal { background: var(--surface); border: 1px solid var(--border-bright); border-radius: 14px; width: 260px; box-shadow: 0 24px 64px rgba(0,0,0,0.6); animation: slideIn 0.15s ease; overflow: hidden; }
