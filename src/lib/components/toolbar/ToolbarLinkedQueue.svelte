<script>
  ...
  function handleEnqueue() {
    showEnqueue = true;
    enqueueValue = '';
    setTimeout(() => enqueueInputEl?.focus(), 50);
  }
  ...
  function handleDequeue() {
    if ($linkedQueueIsEmpty) { toast.error('Queue underflow — queue is empty'); return; }
    pushHistory();
    dequeueLinked();
    pushHistory();
    toast.success('Dequeued head node');
  }
  ...
  onMount(() => {
    const onEnqueue = () => handleEnqueue();
    const onDequeue = () => handleDequeue();
    window.addEventListener('linkedqueue:enqueue', onEnqueue);
    window.addEventListener('linkedqueue:dequeue', onDequeue);
    return () => {
      window.removeEventListener('linkedqueue:enqueue', onEnqueue);
  ...
  <div class="actions">
    ...
    <Tooltip text="Enqueue node">
      <button class="btn btn-primary" onclick={handleEnqueue}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          <path d="M10 7H4M7.5 4.5L10 7l-2.5 2.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        Enqueue
      </button>
    </Tooltip>
    ...
    <Tooltip text={$linkedQueueIsEmpty ? 'Queue is empty' : 'Dequeue head node'}>
      <button class="btn btn-primary" onclick={handleDequeue} disabled={$linkedQueueIsEmpty}>
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          <path d="M4 7h6M4 4.5L1.5 7 4 9.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        Dequeue
      </button>
    </Tooltip>
...
{#if showEnqueue}
  ...
  <div class="modal-overlay" onmousedown={() => showEnqueue = false}>
    <div class="modal" onmousedown={(e) => e.stopPropagation()}>
      <div class="modal-header">
        <span class="modal-title">Enqueue value</span>
        <button class="close-btn" aria-label="Close" onclick={() => showEnqueue = false}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <div class="modal-footer">
        <button class="btn btn-secondary" onclick={() => showEnqueue = false}>Cancel</button>
        <button class="btn btn-primary" onclick={confirmEnqueue}>Enqueue</button>
      </div>
    </div>
  </div>
{/if}
