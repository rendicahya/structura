<script>
  ...
  registerHistoryHandlers(getSnapshotUR, applySnapshotUR);
  import { clearLogUR } from '../../stores/shared/undoRedoLog.js';
  ...
  // editor mock-up, so they're accepted but intentionally unused.
  const { zoom = 1, zoomIn, zoomOut, zoomReset } = $props();

  let showConfirmNew = $state(false);

  // The only way to type is the input bar in the editor mock-up
  // (CanvasUndoRedo), which dispatches `editor:type`. This handler wraps
  // the store op in the undo/redo bracket.
  function doType(raw) {
    if (!raw) {
  ...
  function handleNew() {
    if ($documentText || $undoStack.length || $redoStack.length) {
      showConfirmNew = true;
    } else {
      confirmNewActual();
    }
  ...
  function handleSave() {
    downloadStructure('undo-redo', getSnapshotUR());
  ...
  onMount(() => {
    const onType = (/** @type {CustomEvent<string>} */ e) => doType(e.detail);
    const onUndo = () => {
    ...
    window.addEventListener('editor:type', onType);
    window.addEventListener('editor:undo', onUndo);
    window.addEventListener('editor:redo', onRedo);
    return () => {
      window.removeEventListener('editor:type', onType);
...
<div class="toolbar">
  ...
  <div class="actions">
    ...
    <Tooltip text="Reset the editor and both stacks">
  ...
  <div class="modal-overlay" onmousedown={() => (showConfirmNew = false)}>
    <div class="modal modal-sm" onmousedown={(e) => e.stopPropagation()}>
      <div class="modal-header">
        <span class="modal-title">Reset editor</span>
        <button class="close-btn" aria-label="Close" onclick={() => (showConfirmNew = false)}>
          <Icon name="close" size={14} />
        </button>
      </div>
      <div class="modal-body">
        <p class="confirm-text">Clear the document and both undo/redo stacks?</p>
      </div>
