<script>
  ...
  registerHistoryHandlers(getSnapshotBM, applySnapshotBM);
  import { clearLogBM } from '../../stores/shared/bracketMatchingLog.js';
  ...
  function handleNew() {
    if ($bracketState.steps.length > 0) {
      showConfirmNew = true;
    } else {
      confirmNewActual();
    }
  ...
  function confirmNewActual() {
    ...
    toast.success('Bracket stack reset');
  }

  function handleSave() {
    downloadStructure('bracket-matching', getSnapshotBM());
...
<div class="toolbar">
  ...
  <div class="actions">
    ...
    <Tooltip text="Reset the bracket stack">
  ...
  <div class="modal-overlay" onmousedown={() => (showConfirmNew = false)}>
    <div class="modal modal-sm" onmousedown={(e) => e.stopPropagation()}>
      <div class="modal-header">
        <span class="modal-title">Reset bracket stack</span>
        <button class="close-btn" aria-label="Close" onclick={() => (showConfirmNew = false)}>
          <Icon name="close" size={14} />
        </button>
      </div>
      <div class="modal-body">
        <p class="confirm-text">Clear the current expression and its stack?</p>
      </div>
