<script>
  ...
  function handleSave() {
    ...
    snap._type = 'dll'; // marker untuk load
    const blob = new Blob([JSON.stringify(snap, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'structura-dll-save.json'; a.click();
...
<div class="toolbar">
  <div class="brand">
    ...
    <span class="brand-sub">Doubly Linked List</span>
  </div>
  ...
  <div class="actions">
    ...
    <button class="btn btn-icon" class:active={codeHidden} on:click={() => dispatch('toggleCode')}
