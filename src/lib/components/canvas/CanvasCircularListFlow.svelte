<script>
    ...
    function handleEdit(nodeId, value) {
        pushHistory();
        setNodeValueCircular(nodeId, value);
        pushHistory();
    }

    let flowNodes = $derived(
        $circularListNodes.map((node) => ({
            id: node.id,
            type: "circularlist",
    ...
    function onNodeDragStop({ targetNode }) {
        ...
        pushHistory();
        moveNodeCircular(targetNode.id, targetNode.position.x, targetNode.position.y);
        pushHistory();
    }
