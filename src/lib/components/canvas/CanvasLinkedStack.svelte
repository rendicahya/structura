<script>
  import { linkedStackNodes, topId, linkedStackIsEmpty, garbageCollectLinkedStack, peekLinkedStack } from '../../stores/stack/graphLinkedStack.js';
  ...
  const NODE_W = 130;
  const NODE_H = 64;
  const NODE_GAP = 40; // ruang untuk panah antar node
    ...
    <g style="transform: translate({panX}px, {panY}px) scale({zoom}); transform-origin: 0 0;">
      ...
      {#each stackNodeIds() as nodeId, index}
        ...
        {#if node}
          ...
          >
            <!-- Shadow -->
            <rect x="2" y={y+4} width={NODE_W} height={NODE_H} rx="10" fill="rgba(0,0,0,0.35)"/>
            ...
            <rect
              ...
              rx="10"
              fill="var(--node-bg)"
              stroke={isPeeking ? 'var(--warning)' : isTop ? 'var(--accent)' : 'var(--node-border)'}
              stroke-width={isTop || isPeeking ? 1.8 : 1}
            />

            <!-- Top accent bar -->
            <rect
              x="1" y={y+1} width={NODE_W-2} height="3" rx="2"
              fill={isPeeking ? 'var(--warning)' : isTop ? 'var(--accent)' : 'var(--node-border)'}
              opacity="0.8"
            />

            <!-- var name -->
            <text x={NODE_W/2} y={y+22} text-anchor="middle"
              font-family="var(--font-mono)" font-size="10"
              fill="var(--accent)" font-weight="500"
            >{node.varName}</text>

            <!-- divider -->
            <line x1="12" y1={y+28} x2={NODE_W-12} y2={y+28} stroke="var(--border)" stroke-width="1"/>

            <!-- data -->
            <text x={NODE_W/2} y={y+50} text-anchor="middle"
          ...
          {#if isTop}
            <rect
              x={NODE_W + 10} y={y + NODE_H/2 - 10}
              width="40" height="20" rx="5"
              fill="rgba(78,204,163,0.15)" stroke="var(--success)" stroke-width="1.2"
            />
            <text
              x={NODE_W + 30} y={y + NODE_H/2 + 4}
              text-anchor="middle"
              font-family="var(--font-mono)" font-size="9" font-weight="700"
              fill="var(--success)" letter-spacing="0.8"
            >TOP</text>
            <line
              x1={NODE_W + 10} y1={y + NODE_H/2}
              x2={NODE_W + 1} y2={y + NODE_H/2}
              stroke="var(--success)" stroke-width="1.5"
            />
            <polygon
              points="{NODE_W + 7},{y + NODE_H/2 - 4} {NODE_W + 1},{y + NODE_H/2} {NODE_W + 7},{y + NODE_H/2 + 4}"
              fill="var(--success)"
            />
          {/if}
          ...
          {#if index < stackNodeIds().length - 1}
            ...
            <line
              x1={NODE_W/2} y1={y + NODE_H}
              x2={NODE_W/2} y2={nextY - 4}
          ...
          {#if index === stackNodeIds().length - 1}
            <line x1={NODE_W/2} y1={y+NODE_H} x2={NODE_W/2} y2={y+NODE_H+GROUND_LEN} stroke="var(--text-muted)" stroke-width="1.5"/>
            {#each GROUND_LINES as gl, i}
              <line
                x1={NODE_W/2 - gl.w/2} y1={y+NODE_H+GROUND_LEN+i*7}
                x2={NODE_W/2 + gl.w/2} y2={y+NODE_H+GROUND_LEN+i*7}
      ...
      {#each unreachableNodes as node, index}
        ...
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <g class="node-group unreachable" oncontextmenu={(e) => onNodeContextMenu(e, node.id)}>
          <rect x="2" y={y+4} width={NODE_W} height={NODE_H} rx="10" fill="rgba(0,0,0,0.2)"/>
          <rect x="0" y={y} width={NODE_W} height={NODE_H} rx="10"
            fill="var(--node-bg)" stroke="var(--border)" stroke-width="1" opacity="0.5"
          ...
          >{node.data || 'null'}</text>
          <!-- GC label -->
          <text x={NODE_W + 10} y={y + NODE_H/2 + 4}
      ...
      {#if contextMenu.type === 'canvas'}
        <button class="ctx-item" onclick={handlePushFromMenu}>
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <path d="M6.5 9V4M4 6.5l2.5 2.5 2.5-2.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          Push
        </button>
        <button class="ctx-item" onclick={handlePopFromMenu} disabled={$linkedStackIsEmpty}>
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <path d="M6.5 4v5M4 6.5l2.5-2.5 2.5 2.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
          Pop
        </button>
        <div class="ctx-divider"></div>
        <button class="ctx-item" onclick={() => handlePeekFromMenu(null)} disabled={$linkedStackIsEmpty}>
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
...
<style>
  ...
  .node-group.anim-in { animation: slideDown 0.35s cubic-bezier(0.34, 1.56, 0.64, 1); }
  @keyframes slideDown {
